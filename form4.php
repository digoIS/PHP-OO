 ...
 * ou seja, quando ela é unstancia pela primeira vez
 */
...
class LivrosForm extends TPage
{
	...
	function __construct()
	{
		...
		$panel->put(new TImage('app.images/book.png'), 320, 20);
		
		//coloca o campo título no formulário 
		$panel->put(new TLabel('Titulo'), 100, 40);
		...
		//coloca o campo tema no formulárop
		$panel->put(new TLabel('Tema'), 10, 100);
		$panel->put($tema = new TCombo('tema'), 100, 100);
		
		//cria um vetor com as opções do combo tema
		$items = array();
		...
		$panel->put(new TLabel('Resumo'), 100, 160);
	...
	/**
	 * método onSave()
	 * obtém os dados do formulárioo e salva na base de dados
	 */
	function onSave()
	{
		try {
			//inciia transação com o banco 'pg_livro'
		...
		{ 
			//exibe a mensagm gerada pela exceção
	...
	function onEdit($param)
	{
		...
		catch (Exception $e) //em caso de exceção
		{
			...
			new TMessage('error	', '<b>Erro</b> ' . $e->getMessage());
			//desfdaz todas as alterações do banco de dados
			TTransaction::rollback();
