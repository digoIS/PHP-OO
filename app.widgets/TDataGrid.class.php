...
class TDataGrid extends TTable
{
	...
	{
		parent::__construct();
		$this->class = 'tdatagrid_tabel';
		
		//instancia objeto TStyle
		//este estilo sewrá utilizado para a tabela da datagrid
		$style1 = new TStyle('tdatagrid_table');
		$style1->border_collapse 	= 'separate';
		$style1->font_famlily		= 'arial,verdana,sans-serif';
		...
		if($this->columns)
		{
			...
			foreach($this->columns as $column)
			{
				...
				if ($column->getAction())
				{
					...
					$celula->onmousemouver 	= "this.className='tdatagrid_col_over';";
					$celula->onmouseout 	= "this.className='tdatagrid_col'";
					$celula->onmouseout		= "document.location='$url'"; 
				}
			}
		}
	}
	 ...
	 * @param $object = Obejto que contém os dados
	 */
	public function addItem($object)
	{
		...
		if ($this->actions)
		{
			...
			foreach($this->actions as $action)
			{
				...
				//obtém o campo do objeto quee será passado adiante 
				$key		= $object->$field;
		...
		if ($this->columns)
		{
			...
			foreach($this->columns as $column)
			{
				...
				if($function)
				{
					//aplica a função sobr sobre os dados
